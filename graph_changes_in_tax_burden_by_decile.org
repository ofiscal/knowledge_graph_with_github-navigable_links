...
* DONE [[https://github.com/JeffreyBenjaminBrown/ofiscal/knowledge-graph/blob/master/create_a_new_total_tax_variable.org][Create a new total tax variable.]]
* DONE Make a baseline every time.
:PROPERTIES:
:ID:       b8f2e47c-a378-439b-8836-769065fb41be
:END:
** Bear in mind that because [[https://github.com/JeffreyBenjaminBrown/ofiscal/knowledge-graph/blob/master/document_the_microsimulation.org#the-iva-buckets-are-approximate][the IVA buckets are approximate]], the baseline is approximate.
...
*** purpose
    so that it can [[https://github.com/JeffreyBenjaminBrown/ofiscal/knowledge-graph/blob/master/graph_changes_in_tax_burden_by_decile.org#done-every-time-in-run-makefile-build-two-users][build multiple users]]
...
*** why I decided not to
    ...
    [[https://github.com/JeffreyBenjaminBrown/ofiscal/knowledge-graph/blob/master/graph_changes_in_tax_burden_by_decile.org#run-makefile-should-only-make-the-compare-target-if][targets in run-makefile depend on whether the email is "baseline"]],
    but that's worth it --
...
* DONE [[https://github.com/JeffreyBenjaminBrown/ofiscal/knowledge-graph/blob/master/graph_changes_in_tax_burden_by_decile.org#how-to-draw][Draw]] something that compares to the baseline.
